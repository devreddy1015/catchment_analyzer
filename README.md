...
running. The interface on port 3000 is only a front for the API on port 8000; on
...
| API | http://localhost:8000/api/analyzeContour |
| Interactive API docs | http://localhost:8000/docs |

`docker compose up --build` runs everything in one container on port 8000, UI
...
```bash
curl -X POST http://localhost:8000/api/analyzeArea \
...
```bash
curl "http://localhost:8000/api/places?q=durg"
...
```bash
curl "http://localhost:8000/api/elevation?lat=21.2517&lon=81.2970"
